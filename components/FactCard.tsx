...
export default function FactCard({ fact, variant = 'default' }: FactCardProps) {
  ...
  if (variant === 'lead') {
    return (
      <Link href={`/fact/${fact.slug}`} className="group block border-t-2 border-[#0c0c0b] pt-4">
  ...
  if (variant === 'featured') {
    return (
      <Link href={`/fact/${fact.slug}`} className="group block bg-[#0c0c0b] p-6 h-full hover:bg-stone-900 transition-colors">
        <div className="flex items-center gap-2 mb-3">
          <VerdictBadge verdict={fact.verdict} size="sm" />
